# Copilot Instructions for drawSEM

## Project Overview

drawSEM is a **Vite + React + TypeScript web application** for building visual structural equation models (SEMs) and multilevel models. Users draw nodes (variables, constants, datasets) and paths (relationships) on an SVG canvas, which are then serialized to JSON following the `graph.schema.json` specification.
