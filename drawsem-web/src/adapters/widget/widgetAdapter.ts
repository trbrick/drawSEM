...
/**
 * Configuration passed from R (set via window.drawSEMConfig)
 */
...
declare global {
  interface Window {
    drawSEMConfig?: DrawSEMConfig
  }
}

/**
 * Create a Shiny adapter for R integration
  ...
  return {
    ...
    async load(): Promise<GraphSchema> {
      try {
        ...
        if (window.drawSEMConfig?.initialModel) {
          const initialModel = window.drawSEMConfig.initialModel
...
/**
 * Create adapter with automatic context detection
 * Returns Shiny adapter if in Shiny context, otherwise throws error
 *
